...
rather than a dependency. Configuration comes from the environment:
    ...
    SKEDWORX_SMTP_HOST      smtp.wmsl.ca          (required to send)
    SKEDWORX_SMTP_PORT      587                   (default 587, or 465 with SSL)
    SKEDWORX_SMTP_USER      skedworx@wmsl.ca
    SKEDWORX_SMTP_PASSWORD  ...
    SKEDWORX_SMTP_FROM      "skedworx <skedworx@wmsl.ca>"   (required to send)
    SKEDWORX_SMTP_SSL       1 to connect with SSL instead of STARTTLS
...
def send(to, subject, body):
    ...
    if not smtp_configured():
        sys.stderr.write(
            "skedworx: SMTP is not configured, so this message was NOT sent.\n"
            f"skedworx:   To:      {to}\n"
            f"skedworx:   Subject: {subject}\n"
            f"{body}\n")
        return False
    ...
    try:
        ...
        sys.stderr.write(f"skedworx: could not send mail to {to}: {exc!r}\n")
        return False
...
def send_password_reset(to, reset_url, ttl_minutes):
    ...
    return send(
        to,
        "Reset your skedworx password",
        "Someone asked to reset the password for your skedworx account.\n\n"
