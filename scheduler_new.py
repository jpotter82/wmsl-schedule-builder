...
# -------------------------------
# NEW: Dedicated Doubleheader Optimization Pass
# -------------------------------
def force_minimum_doubleheaders(unscheduled, team_availability, field_availability, team_blackouts, timeslots_by_date,
                                team_stats, doubleheader_count, team_game_days, team_game_slots, team_doubleheader_opponents,
                                used_slots, schedule):
    # For each team below the minimum doubleheader sessions, iterate over all dates
    # and try to schedule an extra doubleheader until the minimum is met.
    for team in list(team_stats.keys()):
        while doubleheader_count[team] < MIN_DOUBLE_HEADERS:
            ...
            for d in sorted(timeslots_by_date.keys()):
                ...
                # If the team already has one game on day d, try to add one in the next slot.
                if games_today == 1:
                    current_slot = team_game_slots[team][d][0]
                    try:
                        idx = sorted_slots.index(current_slot)
                    except ValueError:
                        continue
                    ...
                    if scheduled:
                        break
                # If no game is scheduled today, try to schedule a full doubleheader.
                elif games_today == 0:
                ...
                if scheduled:
                    break
            # If no extra doubleheader could be scheduled for this team, exit the loop.
            if not scheduled:
                break
    return schedule, team_stats, doubleheader_count, team_game_days, team_game_slots, team_doubleheader_opponents, used_slots, unscheduled
...
def main():
    ...
    # Dedicated doubleheader optimization pass.
    (schedule, team_stats, doubleheader_count, team_game_days, team_game_slots,
