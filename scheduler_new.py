...
def schedule_doubleheaders_preemptively(unscheduled, team_availability, field_availability, team_blackouts, timeslots_by_date,
    ...
    for d in sorted(timeslots_by_date.keys()):
        ...
        if len(slots) < 2:
            continue
        # For each team that still needs a doubleheader and is free on this day:
        for team in list(team_stats.keys()):
            ...
            # Look for two consecutive slots with free field entries
            for i in range(len(slots) - 1):
                slot1 = slots[i]
                slot2 = slots[i+1]
                free_fields_slot1 = [entry for entry in field_availability if entry[0].date() == d and entry[1] == slot1 and ((entry[0], slot1, entry[2]) not in used_slots)]
                free_fields_slot2 = [entry for entry in field_availability if entry[0].date() == d and entry[1] == slot2 and ((entry[0], slot2, entry[2]) not in used_slots)]
                if not free_fields_slot1 or not free_fields_slot2:
                    continue

                # Find two distinct matchups (from unscheduled) involving 'team' with different opponents.
                ...
                for combo in itertools.combinations(candidate_matchups, 2):
                    ...
                    if opp1 == opp2:
                        continue
                    # Check opponents' availability for the day:
                    ...
                    if day_of_week not in team_availability.get(opp2, set()) or d in team_blackouts.get(opp2, set()):
                        continue
                    # Ensure opponents are not already scheduled on this date.
                    if team_game_days[opp1].get(d, 0) != 0 or team_game_days[opp2].get(d, 0) != 0:
                        continue
                    # Check weekly game limits for team and opponents:
